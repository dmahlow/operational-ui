...
function roundedUpWidth(el: any): number { return Math.ceil(el.getBoundingClientRect().width) }
function roundedUpHeight(el: any): number { return Math.ceil(el.getBoundingClientRect().height) }
...
abstract class Legend {
  ...
  updateDraw(): void {
    ...
    let legends: any = this.legend.selectAll(`div.${styles.seriesLegend}`)
    ...
    legends
      ...
      .each(withD3Element((d: any, el: HTMLElement): void => {
      ...
      .each(withD3Element((d: any, el: HTMLElement): void => {
        ...
        element.select("div.color")
          .style("background-color", this.colorAccessor)
        element.select("div.name")
          .html(this.labelAccessor)
      }))
